...
def lists_to_dataset_structure(sentences_tokens,sentence_tags,total_token_counter,token_count,label_count,character_count):
    ...
    # print(sentence_tags)
    '''
    [['B-test', 'I-test', 'I-test', 'I-test', 'I-test', 'O', 'O', 'B-problem', 'I-problem', 'I-problem', 'O'],...]
    '''
    # token_count = collections.defaultdict(lambda: 0)  #count freq of keys
    ...
    for idx,sentence in enumerate(sentences_tokens):
        # print(sentence) # ['follow', 'up', 'per', 'skilled', 'nursing', 'facility', 'until', 'discharged', 'to', 'regular', 'residence', '.']
        ...
        for  token_idx,token_i in enumerate(sentence):
                ...
                token_features=[] # currently no features are used
...
class Dataset(object):
    ...
    def _parse_dataset(self, dataset_filepath, dataset_type, sentences_list=[],tags_list=[], Not_here=False):
       ...
       # size_of_features=0
        ...
        if Not_here==False:
          labels, tokens, token_count, label_count, character_count,features,feature_file_name,feature_vector_size \
              =lists_to_dataset_structure(sentences_list,tags_list,total_token_counter,token_count,label_count,character_count)
    ...
    def load_dataset(self,avaliable_datasets_sent,avaliable_datasets_labels, dataset_filepaths, parameters, token_to_vector=None,pretrained_dataset=None):
        ...
        print('Load dataset... \n')
        if parameters['token_pretrained_embedding_filepath'] != '': #vector2.txt
        ...
        token_count['all'] = {} #dict of {'words': count, ... }
        # print(list(token_count['train'].keys())) #
        # exit()
        ...
        # print(token_count['all']) # {'__num__': 26, '.': 55, 'echocardiogram': 2, ...}
        # exit()
