...
def extract_features(tok_sents):
    ...
    prose_feats   = []
    for i,sentence in enumerate(tok_sents):
        # if i == 2:
            # print(sentence) # first sentence = ['b.i.d', '.']
            # exit()
        prose_feats.append(extract_features_sentence(sentence))

    # print(prose_feats)
    '''
    [[{('prev_word_shape', '__x__'): 1, ('prev_word_shape', 'WT-x'): 1, ('prev_next_unigrams-0', '.'): 1, ('prev_prev_
        pos_context-0', '.'): 1, ('prev_pos', 'NNP'): 1, ('prev2', '*'): 1, ('next', '*'): 1, ('next2', '*'): 1}]]
    '''
    return prose_feats
